    ...
    try {
      ...
      for(let rx of recipients){
        for(let sib of rx.parent.children){
          if(sib.type == VT.VBUS){
            let vbus = sib 
            // if we have something that already points at the target, we won't be doing any work... 
            let previouslyDone = undefined 
            // if any of them point at our target endpoint, we might be able to reuse them, 
            for(let ch in vbus.broadcasts){
              if(vbus.broadcasts[ch]){ // these are 'undefined' if they don't exist 
                console.warn(`existing!`)
                console.warn(vbus.broadcasts[ch])
                // let's see if it points at our target... 
                let walk = osap.nr.routeWalk(vbus.broadcasts[ch], vbus)
                if(walk.state == 'incomplete') throw new Error(`previously config'd bus channel looks broken?`)
                // is the thing at the end of that walk... our targ?
                if(walk.path[walk.path.length - 1] == rx){
                  console.warn(`previously config'd route is on ch ${ch}`)
                  previousChannels.push(ch)
                  previouslyDone = ch 
                }
              } 
            } // end sweep-thru-exiting, 
            // if we haven't already found one, build it in the next available, 
            if(previouslyDone == undefined){
              if(previousChannels.length > 0) throw new Error(`looks like a complex diff, my dude`)
              for(let ch in vbus.broadcasts){
                if(vbus.broadcasts[ch]) continue; // pass over existing 
                console.warn(`will setup new bus-to-ep route here at ch ${ch}`)
                // we want a route from the vbus to the endpoint, 
                let routeFromChannel = osap.nr.findRoute(vbus, rx)
                if(!routeFromChannel) throw new Error(`failed to find vbus-to-target route...`)
                // can set that up... 
                await osap.mvc.setVBusBroadcastChannel(vbus.route, ch, routeFromChannel)
                console.warn(`we've allegedly built that route...`)
                break
              }
            }
            // else, we can just blind forward-write a new channel ? 
          }
        }
      }
      return 
